...
export class ValidationController {
    validateRequestBodyNp = async (
        ...
        if (!isValidJSON(JSON.stringify(body))) {
            ...
            res.status(200).send(
                setBadRequestNack(": Invalid request body is not a valid JSON"),
            );
            return;
        }
        ...
        try {
            ...
            res.status(200).send(
                setBadRequestNack("Invalid Action : " + action),
            );
            return;
        }
        try {
    ...
    validateL1Custom = async (
        ...
        try {
            logger.info(
                "Performing L1 custom validations",
                getLoggerMetaData(req),
            );
            const action = getActionParam(req);
            const body = req.body;
    ...
    validateSingleL1 = async (
        ...
        try {
            logger.info(
                "Performing single L1 validations",
                getLoggerMetaData(req),
            );
            const action = getActionParam(req);
            const body = req.body;
            ...
            if (!isValid) {
                ...
                res.status(200).send(
                    setAckResponse(false, req.body, allErrors, code.toString()),
                );
                return;
            }
            logger.info("L1 validations passed", getLoggerMetaData(req));
