    ...
    if (input.config.onlyInvalid === true) {
        ...
        if (res.length === 0) {
            const targetSuccess = totalResults.find(
                (r) => r.testName === "status_validations",
            );
            if (!targetSuccess) {
                                    ...
                                    return [
                                        {
                                            ...
                                            _debugInfo: {
                                                fedConfig: `
{"_NAME_":"REGEX_CONTEXT_BAP_URI","attr":"$.context.bap_uri","reg":["^https:\\/\\/[a-zA-Z0-9-]+(\\.[a-zA-Z0-9-]+)+\\/[a-zA-Z0-9\\-\\/]*\\/?$"],"_RETURN_":"attr follow regex reg","action":["status"],"domain":["ONDC:TRV14"],"version":["2.0.0"]}
`,
                                            },
                                        },
                                    ];
